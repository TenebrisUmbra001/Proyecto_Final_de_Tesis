...
import { registrarSSHInicio, registrarSSHFin } from './db/db';
...
export function montarProxySSH(server: any) {
    ...
    ioServer.on('connection', (socket: any) => {
        ...
        let idAudit: number | null = null;

        socket.on('ssh-connect', (data: { host: string; port?: number; user: string; pass: string; equipoId?: string; usuarioSistema?: string }) => {
            console.log(`[SSH-AUDIT] ${data.usuarioSistema || 'desconocido'} → ${data.user}@${data.host}:${data.port || 22}`);

            registrarSSHInicio(
                data.usuarioSistema || 'desconocido',
                data.user,
                data.equipoId || '',
                data.host
            ).then((id) => { idAudit = id; }).catch(() => {});
        ...
        socket.on('disconnect', () => {
            ...
            if (idAudit) registrarSSHFin(idAudit);
        });
    });
